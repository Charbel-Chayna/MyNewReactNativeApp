...
export const ProductListScreen: React.FC = () => {
  ...
  const styles = React.useMemo(() => getStyles(theme === 'dark'), [theme]);
...
const onRefresh = async () => {
  ...
  return (
    <SafeAreaView testID="product-list-screen" style={styles.container}>
      <Navbar />
  ...
  />
          ...
          <FlatList testID="product-flatlist"
  data={products}
  ...
  renderItem={({ item }) => {
    ...
    return (
      <ProductCard
        ...
        onShare={() => {
    const url = `Cartify://product/${item._id}`;
    Share.share({
      message: `${item.title}\n\n${item.description}\n\n${url}`,
    }).catch(() => Alert.alert('Error', 'Could not share product'));
  }}
      />
