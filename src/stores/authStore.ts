...
export const useAuthStore = create<AuthState>((set) => ({
  ...
  login: async (email, password) => {
    ...
    try {
      ...
      await useCartStore.getState().setCurrentUserEmail(email);
  ...
  logout: async () => {
    ...
    await AsyncStorage.removeItem('refreshToken');
      await useCartStore.getState().setCurrentUserEmail(null);
  ...
  refreshAccessToken: async () => {
    try {
      ...
      console.warn(
        '[authStore] Refresh token failed but not logging out due to transient error.'
      );
